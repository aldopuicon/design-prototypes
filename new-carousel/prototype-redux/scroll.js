  ...
  function start() {
      ...
      if (window._doScroll !== false) {
        ...
        if (carousel) {
          ...
          // If user’s mouse is over the carousel for 5 seconds, show the carousel

          var throttle;
          carousel.addEventListener("mouseover", function(e) {
            if (throttle) clearTimeout(throttle);
            throttle = setTimeout(function(){
              if (mousey < (window.carouselHeight || 45) )
                showCarousel();
            }, 1500);
          });
      ...
      function initialize() {
        ...
        scrollTimer = setTimeout(function() {
          ...
          //if (getViewportDimensions().height + scroll.top < y) {
            lastScrollTop = undefined;
            var magicNumber = window.magicNumber || -210;  // Move a little bit above the target. (This number is arbitrary.)
